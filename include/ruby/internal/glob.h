...
RBIMPL_ATTR_NONNULL(())
/**
 * Identical to rb_glob(), except it  doesn't raise but return opaque exception
 * states instead.
 *
 * @param[in]  pattern  A glob pattern.
...
RBIMPL_ATTR_NONNULL(())
/**
 * Identical to rb_glob(), @shyouhei  currently suspects.  Historically you had
 * to  call  this  function  instead  of  rb_glob()  if  the  pattern  included
 * "{x,y,...}" syntax.  However since commit 0f63d961169989a7f6dcf7c0487fe29da,
 * rb_glob()  also supports  that syntax.   It seems  as of  writing these  two
