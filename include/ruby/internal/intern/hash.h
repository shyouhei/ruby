 ...
 *
 * @param[in]  key       A key of the table.
 * @param[in]  existing  Value associated with `key` in hash1.
 * @param[in]  incoming  Value associated with `key` in hash2.
 * @return     Either one of the passed values to take.
 */
typedef VALUE rb_hash_update_func(VALUE key, VALUE existing, VALUE incoming);
