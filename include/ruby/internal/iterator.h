 ...
 *     const int n = get_n(self);
 *
 *     for (int i=0; i<n; i++) {
 *         VALUE v = get_v(self, i);
 *
 *         rb_yield(v);
 *     }
 *     return self;
 ...
 * @internal
 *
 * This API  is not yet  marked deprecated because there  seems to be  many use
 * cases of it in the wild.  But maybe we will someday.
 */
