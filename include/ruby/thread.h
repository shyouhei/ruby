 ...
 * static inline void* callback(void* buf) {
 *   int len;
 *   void* tmp = ruby_xmalloc(BUFSIZ);
 *   void* ret = ruby_xmalloc(sizeof(ssize_t));  // (a)
 *   ssize_t n = read(fd, tmp, BUFSIZ);          // (b)
 *   memcpy(buf, tmp, n);                        // (c)
 ...
 * way more  harmful because that should  nuke what was read.   To reroute this
 * kind of problem you should set this flag.  And check interrupts elsewhere at
 * your own risk.
 */
